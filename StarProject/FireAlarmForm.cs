...
namespace FireAlarm
{
    public partial class FireAlarmForm : Form
    {
        ...
        private void MainDialog_Load()
        {
            ...
            ObixAddress = ConfigFile.ReadString("OBIX", "Address", "192.168.1.112");
            ...
            //数据库地址
            dbStr = ConfigFile.ReadString("MYSQL", "dbStr", "Database=test;Data Source=192.168.1.112;User Id=root;Password=root;pooling=false;CharSet=utf8;port=3306");
            //与obix建立连接 需放开注释
            connectServer();
         //   String area = "Fire";
           // String point = "F1";
                    ...
                    if(PointName!=null && "true {overridden} @ 1".Equals(status)&&i==0)
                        {
                            ...
                            //String guid = "";
                                 //区域及点位查出guid
                            String guid=SDataHepler.getGuid("111","2222");
                                if (guid != null && guid == "")
                                {
                                    //调用 打开视频方法
